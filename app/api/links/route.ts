...
import { generateFancyQr } from "@/lib/utils/qr-generator";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === "VCARD" && (!vcard || !vcard.firstName || !vcard.lastName)) {
      return NextResponse.json(
        { error: "First name and last name are required for vCard" },
        { status: 400 }
      );
    }
    ...
    if (
      type === "APP_DOWNLOAD" &&
      !appDownload?.iosUrl &&
      !appDownload?.androidUrl
    ) {
      return NextResponse.json(
        { error: "At least one app store URL is required" },
        { status: 400 }
      );
    }
    ...
    const qrSvg = await generateFancyQr(redirectUrl);
